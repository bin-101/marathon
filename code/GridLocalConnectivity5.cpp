/*
事前計算しようとすると、約1.6*10^7回計算する必要があるので、大体の場合、事前計算する必要はなさそう
GridLocalConnectivity3を枝刈りに利用している
事前計算を実装すると、ローカルで約1300msかかった(3*3の枝刈りは利用していない)
verify:https://atcoder.jp/contests/ahc024/submissions/46389874
*/
//#define NDEBUG
...
struct GridLocalConnectivity5{
    array<int,24> dh24;
    array<int,24> dw24;

    const int size=(1<<24);

    const array<int,4> near={-5,1,5,-1};

    array<bool,25> state;
    array<bool,25> visited;

    array<int,8> Z={6,7,8,11,12,15,16,17};

    GridLocalConnectivity5(){
        int id=0;
        for(int h=-2;h<=2;h++){
            for(int w=-2;w<=2;w++){
    ...
    void dfs(int x){
        ...
        for(int d:near){
            int nx=x+d;
            if(nx<0 or nx>=25) continue;
            if(x%5==4 and d==1) continue;
            if(x%5==0 and d==-1) continue;
            if(state[nx] and visited[nx]==false){
                visited[nx]=true;
                dfs(nx);
            }
        }
    ...
    bool is_connected(int S){
        ...
        int X=0;
        for(int i=0;i<8;i++){
            if(bitUP(S,Z[i])) X|=(1<<i);
        }
        if(gridLocalConnectivity3.is_connected(X)) return true;
        ...
        int near_block=-1;
        for(int d1:near){
            if(state[d1+12]) cnt++,near_block=d1+12;
        }
        if(cnt<=1){
            return true;
        }
        dfs(near_block);
        for(int d1:near){
            if(state[d1+12] and visited[d1+12]==false) return false;
        }
        return true;
    }
};
...
GridLocalConnectivity5 gridLocalConnectivity5;
...
bool transition(int h,int w,int post,vector<vector<int>> &grid){
    ...
    /*for(int dir=0;dir<8;dir++){
        int nh=p.h+gridLocalConnectivity.dh8[dir];
        int nw=p.w+gridLocalConnectivity.dw8[dir];
        if(grid[nh][nw]==pre) idx|=(1<<dir);
    }*/
    for(int dir=0;dir<24;dir++){
        int nh=p.h+gridLocalConnectivity5.dh24[dir];
        int nw=p.w+gridLocalConnectivity5.dw24[dir];
        if(Place(nh,nw).out_grid()){
            if(pre==0) idx|=(1<<dir);
        }
        else if(grid[nh][nw]==pre) idx|=(1<<dir);
    }

    return gridLocalConnectivity5.is_connected(idx);
}
