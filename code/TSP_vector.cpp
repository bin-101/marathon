/*
マラソンマッチテンプレート
verify: https://atcoder.jp/contests/tessoku-book/submissions/42931649
*/
#define NDEBUG
...
class TSP{
public:
    vector<vector<T>> distance_;
    vector<int> ans_;
    int n_;
    vector<int> reuse_;//使いまわす
    vector<int> perm_;//使いまわす　常に[0,n_-1]の順列
    vector<vector<int>> order_; //[i][j]: 頂点iからj番目に近い頂点
    vector<int> place_; //ans_[place_[i]]=i
    T inf_;
    ...
    TSP(int n):n_(n),inf_(numeric_limits<T>::max()),distance_(n),ans_(n),reuse_(n),perm_(n),
        order_(n),place_(n){
        for(int i=0;i<n_;i++){
        ...
        for(int i=0;i<n_;i++){
            order_[i].resize(n_-1);
            for(int j=0;j<n_;j++){
                if(j<i) order_[i][j]=j;
                else if(j>i) order_[i][j-1]=j;
            }
        }
    }
    ...
    T dist(int i,int j,const vector<int> &perm){
        return distance_[perm[i]][perm[j]];
    }

    T calc_sum_dist(const vector<int> &perm){
    ...
    //debug用 O(n_)
    //reuse_が1になってしまう
    bool is_perm(const vector<int> &v){
        reuse_.assign(n_,0);
        for(int i=0;i<n_;i++){
...
void solve(){
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
            tsp.set(i,j,sqrt((x[i]-x[j])*(x[i]-x[j])+(y[i]-y[j])*(y[i]-y[j]))*10000);
        }
    }
    assert(tsp.n_);
    tsp.build_order();
    ...
    int start=-1;
    for(int i=0;i<n;i++){
        if(tsp.ans_[i]==0) start=i;
    }

    for(int i=0;i<n;i++){
        cout<<tsp.ans_[(i+start)%n]+1<<endl;
    }
    cout<<1<<endl;
