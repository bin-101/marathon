...
struct WeightedRandom{
    ...
    void add(Name name,Weight weight){
        assert(weight>=0);
        idx_[name]=weight_.size();
        weight_.push_back(weight);
        name_.push_back(name);
        if(sum_.size()) sum_.push_back(sum_.back()+weight);
        else sum_.push_back(weight);
    }
    //Rand32の方がいいかも
    Name random(){
        assert(sum_.size()>=1);
        Weight x=Rand64()%u64(ceil(sum_.back()));
        int left=-1,right=sum_.size()-1;
        while(right-left>1){
            int mid=(right+left)/2;
            if(sum_[mid]<=x) left=mid;
            else right=mid; 
        }
        return name_[right];
    }
    void change(Name name,Weight weight){
        weight_[idx_[name]]=weight;
        for(int i=idx_[name];i<sum_.size();i++){
            sum_[i]=sum_[i-1]+weight_[i];
        }
    }
    void erase(Name name){
        int erase_id=idx_[name];
        Weight erase_weight=weight_[erase_id];

        for(int i=erase_id+1;i<sum_.size();i++){
            idx_[name_[i]]--;
            name_[i-1]=name_[i];
            weight_[i-1]=weight_[i];
            sum_[i-1]=sum_[i]-erase_weight;
        }

        name_.resize(name_.size()-1);
        weight_.resize(weight_.size()-1);
        sum_.resize(sum_.size()-1);
        idx_.erase(name);
    }
};
