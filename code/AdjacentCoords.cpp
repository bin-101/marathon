...
//https://atcoder.jp/contests/rcl-contest-2021-long/submissions/25805405
struct AdjacentCoords{
    using PA=DynamicArray<Place,4>;
    vector<PA> table;

    AdjacentCoords(int Height,int Width):table(Height*Width){
        for(int id=0;id<Height*Width;id++){
            Place p(id);
            for(int dir=0;dir<4;dir++){
                auto np=p.next_place(dir);
                if(np.out_grid()) continue;
                table[id].push_back(np);
            }
        }
    }
    const PA& operator()(int place_id)const{
        return table[place_id];
    }
    const PA& operator()(Place place)const{
        return table[place.idx()];
    }
};

void solve(){
    AdjacentCoords adjacentCoords(10,10);
    auto array=adjacentCoords(Place(0,0));
    array[0]=Place(0,0);
    for(Place a:array){
        cout<<a<<endl;
    }
