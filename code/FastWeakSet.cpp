/*
FastWeakSet
参考://https://github.com/colun/mmlang/blob/master/lib/fast_weak_set64.h
verify:https://atcoder.jp/contests/rco-contest-2018-qual/submissions/46402011
*/
#define NDEBUG
...
constexpr int num_map=100;
constexpr int num_use_map=8;
constexpr int Height=50;
constexpr int Width=50;
constexpr int max_length_command=2500;

//→　↓ ← ↑
int dhw[4]={1,Height,-1,-Height};
...
struct Place{
    int id;
    Place(){}
    Place(int h,int w):id(h*Width+w){
...
struct Map{
    ...
    DynamicArray<Place,3000> pre_now;
    DynamicArray<char,3000> pre_state;

    bool is_wall(Place p){
        return map_[p.id]=='#';
    }
    bool is_coin(Place p){
        return map_[p.id]=='o';
    }
    bool is_trap(Place p){
        return map_[p.id]=='x';
    }

    //実際に行動する
    void command(int dir){
        pre_now.push_back(now);

        if(traped){
            pre_state.push_back('.');
            return;
        }
        Place next=now.next_place(dir);
        if(is_wall(next)){
            pre_state.push_back('.');
            return;
        }
        now=next;
        pre_state.push_back(map_[next.id]);
    ...
    void re_command(){
        map_[now.id]=pre_state.back();

        if(pre_state.back()=='x' and now!=pre_now.back()){
            score+=5000;
            traped=false;
    ...
    pair<int,u64> try_command(int dir){
        if(traped){
            return {score,hash_map[now.id]};
        }
        Place next=now.next_place(dir);
        u64 hash=hash_map[next.id];
        if(is_wall(next)){
            return {score,hash_map[now.id]};
        }
...
//https://github.com/colun/mmlang/blob/master/lib/fast_weak_set64.h
struct FastWeakSet{
    const u32 size=(1<<20);
    const u32 mask=size-1;
    array<u32,1<<20> memo{};
    DynamicArray<u32,1<<20> keys;
    ...
    struct Tree{
        ...
        void select_candidate(uint beam_width){
            ...
            for(int i = 0;(i<candidate_size and cnt<beam_width);i++){
                if(Set.count(candidate[i].hash)==0){
                    cnt++;

                    Set.insert(candidate[i].hash);
